...
- [ ] (Super High v79) Merged-cell ridge — U-shaped merges (and presumably C-shaped, inverted-U, inverted-C, and other concave-perimeter merges) lose the rounded convex corners that the merged opening had pre-ridge. **Not reproduced 2026-06-07** with the three U topologies I tried (2-row narrow U, 3-row tall-arm U, 2-row U with thick ridge t=5) — all rendered with properly rounded convex corners on both the outer tips of the arms and the notch-facing inner-convex corners. Need a specific repro: which preset/test case, grid dimensions, cells in the merge, ridge height/thickness, and any non-default cell-corner-radius or cell-edge-slope settings. Possibly the bug only shows up with specific parameter combinations the existing test corpus doesn't cover.
