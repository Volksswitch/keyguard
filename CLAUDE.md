...
- [ ] (Super High v79) Merged-cell ridge — square/rectangular merge produces a spurious free-floating small ridge floor in the middle of the opening (instead of just the outer perimeter ridge). Visible when merging a 2×2 (or larger) rectangular block of cells.
- [ ] (Super High v79) Merged-cell ridge — U-shaped merges (and presumably C-shaped, inverted-U, inverted-C, and other concave-perimeter merges) lose the rounded convex corners that the merged opening had pre-ridge. The outer convex corners at the open end of the U come out as sharp 90° corners after ridge application; they should keep the original rounded corner profile. Root cause likely the same as the square-merge bug: merge-perimeter recognition is too narrow, only handling L-shape and cross-shape patterns explicitly. Needs a more general perimeter walker that treats any merge shape as a closed polygon with convex/concave corners classified per vertex.
